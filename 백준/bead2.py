...
def left(red, blue, level):
    ...
    for i in range(0, M):
        if [ry, rx -i] == O:
            ...
            rx -= (i - 1)
            break
    simulation([ry, rx], [by, bx], level)




def right(red, blue, level):
    [ry, rx] = red
    [by, bx] = blue
    for i in range(0, M):
        if [ry, rx + i] == O:
    ...
    for dy, dx in direction:
        ...
        if [dy, dx] == [0, 1]:
            if ([rny, rnx] in lstob and [bny, bnx] == [ry, rx]) or ([bny, bnx] in lstob and [rny, rnx] == [by, bx]):
                ...
                right(red, blue, level + 1)
        if [dy, dx] == [0, -1]:
            if ([rny, rnx] in lstob and [bny, bnx] == [ry, rx]) or ([bny, bnx] in lstob and [rny, rnx] == [by, bx]):
                pass
            else:
                left(red, blue, level + 1)
        if [dy, dx] == [1, 0]:
            if ([rny, rnx] in lstob and [bny, bnx] == [ry, rx]) or ([bny, bnx] in lstob and [rny, rnx] == [by, bx]):
                pass
            else:
                down(red, blue, level + 1)
        if [dy, dx] == [-1, 0]:
            if ([rny, rnx] in lstob and [bny, bnx] == [ry, rx]) or ([bny, bnx] in lstob and [rny, rnx] == [by, bx]):
                pass
            else:
